		...
		var btnCancel = '<button class="btn hollow btn-cancel" data-close="" aria-label="關閉彈窗" type="button">取消</button>'; var
			...
			PopupModule = /*#__PURE__*/function () {
				function PopupModule(type) {
					...
					key: "_promise", value: function _promise(domCache) {
						var _this = this;
			...
			[].forEach.call(searchGroup, function (item) {
				...
				var searchCheckValue = function searchCheckValue(el) {
					...
					if (target.value.trim().length > 0) {
						...
						$('.hd-search__result__block[data-layout="recommend"]').hide();
						$('.hd-search__result__block[data-layout="recommend-searchs"]').show();
					} else {
						searchElClassRemove(isSearching);
						btnSubmit.disabled = true;
						$('.hd-search__result__block[data-layout="recommend"]').show();
						$('.hd-search__result__block[data-layout="recommend-searchs"]').hide();
					}
				};
			...
			[].forEach.call(countBoxEls, function (el) {
				...
				var maxHandler = function maxHandler(value) {
					if (value) {
						...
						$('.pd-side-count__note.blindBox-use p.error').text("已達單次開盒上限");
						$('.pd-side-count__note.game-use p.error').text("已達單次放入上限");

					} else {
						btnPlus.disabled = false;
						$('.pd-side-count__note p.error').text("");
						$('.pd-side-count__note.blindBox-use p.error').text("");
						$('.pd-side-count__note.game-use p.error').text("");
					}
				};
